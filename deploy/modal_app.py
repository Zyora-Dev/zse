...
def test_attention():
    ...
    seq_len_large = 2048
    query_large = torch.randn(
        1, num_heads, seq_len_large, head_dim, device="cuda", dtype=torch.float16
    )
    key_large = torch.randn(
        1, num_heads, seq_len_large, head_dim, device="cuda", dtype=torch.float16
    )
    value_large = torch.randn(
        1, num_heads, seq_len_large, head_dim, device="cuda", dtype=torch.float16
    )
    ...
    key_cache = torch.randn(
        num_blocks_total, num_kv_heads, block_size, head_dim, device="cuda", dtype=torch.float16
    )
    ...
    context_lens = torch.randint(
        256, max_context_len, (num_seqs,), dtype=torch.int32, device="cuda"
    )
...
def benchmark_attention(
    ...
    def _flash_attention_kernel_v2(
        ...
        num_heads = (
            tl.num_programs(1) // tl.num_programs(2)
            if tl.num_programs(2) > 0
            else tl.num_programs(1)
        )
        ...
        for block_n in range(num_blocks):
            ...
            k = tl.load(
                K_block_ptr + offs_n_curr[:, None] * stride_kn + offs_k[None, :] * stride_kk,
                mask=(offs_n_curr[:, None] < N_CTX) & (offs_k[None, :] < HEAD_DIM),
                other=0.0,
            )
            ...
            v = tl.load(
                V_block_ptr + offs_n_curr[:, None] * stride_vn + offs_k[None, :] * stride_vk,
                mask=(offs_n_curr[:, None] < N_CTX) & (offs_k[None, :] < HEAD_DIM),
                other=0.0,
            )
    ...
    for seq_len in seq_lengths_list:
        for batch_size in batch_sizes_list:
            ...
            print(
                f"{seq_len:>6} {batch_size:>6} | {zse_ms:>7.2f}ms {sdpa_ms:>7.2f}ms {speedup:>7.2f}x | {memory_mb:>9.1f}MB"
            )
    ...
    class SimpleLlamaLayer(nn.Module):
        def __init__(self, config):
            ...
            self.post_attention_layernorm = RMSNorm(
                config["hidden_size"], config.get("rms_norm_eps", 1e-5)
            )
...
def test_streaming_loader(model_name: str = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"):
    ...
    class LlamaAttention(nn.Module):
        def __init__(self, config, device=None):
            ...
            self.q_proj = nn.Linear(
                self.hidden_size, self.num_heads * self.head_dim, bias=False, device=device
            )
            self.k_proj = nn.Linear(
                self.hidden_size, self.num_kv_heads * self.head_dim, bias=False, device=device
            )
            self.v_proj = nn.Linear(
                self.hidden_size, self.num_kv_heads * self.head_dim, bias=False, device=device
            )
            self.o_proj = nn.Linear(
                self.num_heads * self.head_dim, self.hidden_size, bias=False, device=device
            )
    ...
    class LlamaMLP(nn.Module):
        def __init__(self, config, device=None):
            ...
            self.gate_proj = nn.Linear(
                config["hidden_size"], config["intermediate_size"], bias=False, device=device
            )
            self.up_proj = nn.Linear(
                config["hidden_size"], config["intermediate_size"], bias=False, device=device
            )
            self.down_proj = nn.Linear(
                config["intermediate_size"], config["hidden_size"], bias=False, device=device
            )
    ...
    class LlamaLayer(nn.Module):
        def __init__(self, config, device=None):
            ...
            self.input_layernorm = RMSNorm(
                config["hidden_size"], config.get("rms_norm_eps", 1e-5), device=device
            )
            self.post_attention_layernorm = RMSNorm(
                config["hidden_size"], config.get("rms_norm_eps", 1e-5), device=device
            )
    ...
    class LlamaModel(nn.Module):
        def __init__(self, config, device=None):
            ...
            self.layers = nn.ModuleList(
                [LlamaLayer(config, device=device) for _ in range(config["num_hidden_layers"])]
            )
            self.norm = RMSNorm(
                config["hidden_size"], config.get("rms_norm_eps", 1e-5), device=device
            )
    ...
    print(
        f"   ✅ Meta init used: {(post_meta_mem['allocated'] - pre_meta_mem['allocated']):.3f} GB (should be ~0)"
    )
...
def test_continuous_batching():
    ...
    while scheduler.running or scheduler.waiting:
        ...
        if iterations % 50 == 0:
            print(
                f"   Iteration {iterations}: Running={len(scheduler.running)}, "
