...
def benchmark_fused_kernels():
    ...
    try:
        ...
        print(
            f"  INT8 fused:  {bench_results['int8_fused_ms']:.3f} ms ({bench_results['int8_speedup']:.2f}x vs FP16)"
        )
        print(
            f"  INT4 fused:  {bench_results['int4_fused_ms']:.3f} ms ({bench_results['int4_speedup']:.2f}x vs FP16)"
        )
    ...
    for in_feat, out_feat in sizes:
        ...
        try:
            ...
            q_linear_unfused_int4 = QuantizedLinear.from_float(
                linear_fp16, QuantType.INT4, group_size=128
            )
            q_linear_fused_int4 = FusedQuantizedLinear.from_float(
                linear_fp16, "int4", group_size=128
            )
    ...
    if "synthetic" in results:
        ...
        print(
            f"  INT8 fused:     {s['int8_fused_ms']:.3f} ms ({1 / s['int8_speedup'] * 100:.0f}% of FP16 speed)"
        )
        print(
            f"  INT4 fused:     {s['int4_fused_ms']:.3f} ms ({1 / s['int4_speedup'] * 100:.0f}% of FP16 speed)"
        )
    ...
    try:
        ...
        with torch.no_grad():
            outputs = model_int8.generate(
                **inputs, max_new_tokens=tokens_to_generate, do_sample=False
            )
        torch.cuda.synchronize()
        elapsed = time.perf_counter() - start
