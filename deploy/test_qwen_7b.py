...
"""
    ...
    print(f"   {'-' * 75}")
    print(
        f"   {'Model Memory':<30} {format_bytes(fp16_allocated):>15} {format_bytes(int8_allocated):>15} {(1 - int8_allocated / fp16_allocated) * 100:>14.1f}%"
    )
    print(
        f"   {'Peak Generation':<30} {format_bytes(fp16_gen_peak):>15} {format_bytes(int8_gen_peak):>15} {(1 - int8_gen_peak / fp16_gen_peak) * 100:>14.1f}%"
    )
    ...
    print(f"   {'-' * 75}")
    print(
        f"   {'Tokens/sec':<30} {len(fp16_tokens) / fp16_gen_time:>15.1f} {len(int8_tokens) / int8_gen_time:>15.1f} {(int8_gen_time / fp16_gen_time - 1) * 100:>+14.1f}%"
    )
    print(
        f"   {'Avg Latency (ms)':<30} {sum(latencies) / len(latencies):>15.1f} {sum(latencies_int8) / len(latencies_int8):>15.1f} {(sum(latencies_int8) / len(latencies_int8)) / (sum(latencies) / len(latencies)) * 100 - 100:>+14.1f}%"
    )
...
def test_memory_advisor():
    ...
    for name, params in models:
        ...
        best_fit = (
            fits_int4 if fits_int4 == "✅" else (fits_int8 if fits_int8 == "✅" else fits_fp16)
        )

        print(
            f"   {name:<25} {format_bytes(fp16_size):>12} {format_bytes(int8_size):>12} {format_bytes(int4_size):>12} {best_fit:>10}"
        )
    ...
    for name, params in models:
        ...
        if fp16_size * 1.2 < gpu_free:
            ...
            print(
                f"   {name}: ❌ Won't fit - need {format_bytes(int4_size * 1.2 - gpu_free)} more VRAM"
            )
