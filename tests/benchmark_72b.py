...
# HuggingFace token from environment for fast downloads
...
def install_deps():
    ...
    subprocess.run([sys.executable, "-m", "pip", "install", "-q", "--upgrade"] + deps)
...
def main():
    ...
    try:
        ...
        print("(First run downloads ~150GB, subsequent runs use cache)")
    ...
    try:
        print("[SETUP] Creating 72B INT4 .zse file (simulating pre-quantized weights)...")
        
        zse_path = "/tmp/qwen72b.zse"
        
        # 72B architecture (Qwen2.5-72B specs)
        hidden_size = 8192
        ...
        # INT4 packed layers (2 values per byte)
    ...
    try:
        ...
        print("Downloading GGUF (~42GB, may take a while)...")
        ...
        print(f"✅ GGUF ready: {gguf_size:.2f} GB")
        
        print("\n[BENCHMARK] Loading GGUF to GPU...")
        start_time = time.time()
    ...
    # =========================================================================
    print("\n" + "=" * 70)
    print("BENCHMARK RESULTS: 72B Model Cold Start")
    print("=" * 70)
