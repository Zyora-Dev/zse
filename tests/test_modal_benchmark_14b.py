"""ZSE vs vLLM Benchmark — Qwen2.5-14B-Instruct on A100-80GB.
...
ZSE uses INT4 quantization, vLLM uses FP16.
"""

import sys
import modal

app = modal.App("zse-vs-vllm-14b")
...
def benchmark_vllm():
    ...
    results = {"engine": "vLLM", "model": MODEL_ID}

    print("=" * 70)
    print("vLLM BENCHMARK — Qwen2.5-14B-Instruct (FP16) on A100-80GB")
    print("=" * 70)
    ...
    results["output"] = output_text[:200]
    ...
    # --- Concurrent throughput (4 requests) ---
    print(f"\n[4] Concurrent throughput (4 requests)...")
    prompts_4 = [
    ...
    sp_4 = SamplingParams(temperature=0.0, max_tokens=100)
    # Warmup
    _ = llm.generate(prompts_4, sp_4)
    # Timed
    t0 = time.monotonic()
    out_4 = llm.generate(prompts_4, sp_4)
    t_4 = time.monotonic() - t0
    total_tok_4 = sum(len(o.outputs[0].token_ids) for o in out_4)
    tps_4 = total_tok_4 / t_4
    print(f"     {total_tok_4} tokens in {t_4:.2f}s = {tps_4:.1f} tok/s aggregate")
    results["concurrent_4_tps"] = round(tps_4, 1)
...
def benchmark_zse():
    ...
    results = {"engine": "ZSE", "model": MODEL_ID}

    print("=" * 70)
    print("ZSE BENCHMARK — Qwen2.5-14B-Instruct (INT4) on A100-80GB")
    print("=" * 70)
    ...
    # Raw decode timing (bypass scheduler)
    print("     Raw decode timing...")
    runner = engine._model_runner
    try:
        raw_tokens = tokenizer.encode("Explain the theory of relativity in detail")
        raw_seq = 8888
    ...
    # --- Concurrent throughput (4 requests) ---
    print(f"\n[4] Concurrent throughput (4 simultaneous requests)...")
    concurrent_prompts = [
        "Write about solar energy.",
        "Write about wind energy.",
        "Write about nuclear energy.",
    ...
    print(f"     {total_conc} tokens in {t_conc:.2f}s = {conc_tps:.1f} tok/s aggregate")
    for i in range(4):
        n = len(conc_tokens[i])
        print(f"     Req {i}: {n} tokens")
    results["concurrent_4_tps"] = round(conc_tps, 1)
...
def main():
    print("=" * 70)
    print("ZSE vs vLLM BENCHMARK — Qwen2.5-14B-Instruct on A100-80GB")
    print("=" * 70)
    print()

    # Run both in parallel
    print("Launching both benchmarks...")
    vllm_handle = benchmark_vllm.spawn()
    zse_handle = benchmark_zse.spawn()

    print("Waiting for vLLM...")
    vllm_results = vllm_handle.get()
    print("vLLM done.\n")

    print("Waiting for ZSE...")
    zse_results = zse_handle.get()
    print("ZSE done.\n")

    # --- Comparison Table ---
    print("\n" + "=" * 70)
    print("BENCHMARK RESULTS — Qwen2.5-14B-Instruct on A100-80GB")
    print("=" * 70)

    v = vllm_results
    z = zse_results

    print(f"\n  {'Metric':<30} {'vLLM (FP16)':<20} {'ZSE (INT4)':<20} {'Winner':<15}")
    print(f"  {'-'*30} {'-'*20} {'-'*20} {'-'*15}")
    ...
    speedup = f"({vc/zc:.1f}x)" if zc > 0 and zc < vc else ""
    print(f"  {'Cold start':<30} {vc}s{'':<14} {zc}s{'':<14} {winner} {speedup}")

    # Single-request throughput
    vt = v.get('avg_tps', v.get('tps', 0))
    zt = z.get('avg_tps', z.get('tps', 0))
    winner = "ZSE" if zt > vt else "vLLM"
    print(f"  {'Single req tok/s (avg)':<30} {vt:<20} {zt:<20} {winner}")

    # Raw decode (ZSE only)
    zr = z.get('raw_tps', 0)
    if zr:
        print(f"  {'Raw decode tok/s':<30} {'N/A':<20} {zr:<20}")

    # Concurrent throughput
    vc4 = v.get('concurrent_4_tps', 0)
    zc4 = z.get('concurrent_4_tps', 0)
    winner = "ZSE" if zc4 > vc4 else "vLLM"
    print(f"  {'4-concurrent tok/s':<30} {vc4:<20} {zc4:<20} {winner}")
    ...
    print(f"  {'VRAM used':<30} {vv} GB{'':<13} {zv} GB{'':<13} {winner} {savings}")

    # Weights
    zw = z.get('vram_weights_gb', 0)
    print(f"  {'Weight VRAM':<30} {'~28 GB (FP16)':<20} {f'{zw} GB (INT4)':<20} ZSE")

    # Dependencies
    print(f"  {'Dependencies':<30} {'PyTorch+Triton':<20} {'Zero':<20} ZSE")
    ...
    print(f"{'='*70}")
    if zc < vc and zc > 0:
        print(f"  Cold start: ZSE {vc/zc:.1f}x faster ({zc}s vs {vc}s)")
    if zv < vv and zv > 0:
        print(f"  VRAM: ZSE {vv/zv:.1f}x less ({zv}GB vs {vv}GB)")
    if zt > 0 and vt > 0:
        print(f"  Single throughput: ZSE {zt} vs vLLM {vt} tok/s")
    if zc4 > 0 and vc4 > 0:
        print(f"  Concurrent throughput: ZSE {zc4} vs vLLM {vc4} tok/s")
    print(f"  Dependencies: ZSE=ZERO, vLLM=PyTorch+Triton+CUDA toolkit")
    print()
