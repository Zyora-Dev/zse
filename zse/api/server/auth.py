...
class APIKeyManager:
        ...
        if self.keys_file.exists():
            try:
                with open(self.keys_file) as f:
                    data = json.load(f)
                    self.keys = {k: APIKey.from_dict(v) for k, v in data.get("keys", {}).items()}
    ...
    def _save_keys(self) -> None:
        ...
        with open(self.keys_file, "w") as f:
            json.dump(
                {"enabled": self._enabled, "keys": {k: v.to_dict() for k, v in self.keys.items()}},
                f,
                indent=2,
            )
        # Secure the file
    ...
    def create_key(
        self, name: str, permissions: Optional[List[str]] = None, rate_limit: Optional[int] = None
    ) -> str:
...
async def verify_api_key(
    ...
    internal_routes = [
        ...
        "/api/chat/",
        "/api/rag/",
        "/api/tools/",  # Playground APIs
        "/api/stats",
        "/api/analytics",
        "/api/models",  # Dashboard APIs
    ...
    if api_key.rate_limit is not None and api_key.rate_limit > 0:
        ...
        allowed, current, reset_seconds = rate_limiter.check_and_record(
            api_key.key_hash, api_key.rate_limit
        )
...
def require_permission(permission: str):
    ...
    async def check_permission(api_key: Optional[APIKey] = Security(verify_api_key)):
        if api_key and "*" not in api_key.permissions and permission not in api_key.permissions:
            raise HTTPException(
                status_code=403, detail=f"Permission denied. Required: {permission}"
            )
        return api_key
