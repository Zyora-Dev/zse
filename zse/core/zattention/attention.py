...
class zAttention(nn.Module):
    ...
    def __init__(self, config: AttentionConfig):
        ...
        if self._backend == AttentionBackend.TRITON and not self._triton_available:
            ...
            self._backend = (
                AttentionBackend.TRITON if self._triton_available else AttentionBackend.TORCH
            )
    ...
    def forward(
        ...
        """
        if is_prefill:
            assert key is not None and value is not None, "key and value required for prefill"
            return self.prefill(query, key, value, attention_mask)
        else:
            assert all(
                x is not None for x in [key_cache, value_cache, block_tables, context_lens]
