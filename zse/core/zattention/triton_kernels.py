...
if TRITON_AVAILABLE:
    ...
    def _paged_attention_v2_kernel(
        ...
        if start_block >= num_blocks:
            ...
            tl.store(
                exp_sums_ptr + seq_idx * stride_esn + head_idx * stride_esh + partition_idx, 0.0
            )
            tl.store(
                max_logits_ptr + seq_idx * stride_esn + head_idx * stride_esh + partition_idx,
                float("-inf"),
            )
            return
    ...
    def _flash_attention_kernel(
        ...
        # Load query block
        q = tl.load(
            q_ptrs, mask=(offs_m[:, None] < seq_len) & (offs_d[None, :] < head_dim), other=0.0
        )
        ...
        for start_n in range(0, seq_len, BLOCK_N):
            ...
            k = tl.load(
                k_ptrs, mask=(offs_n[:, None] < seq_len) & (offs_d[None, :] < head_dim), other=0.0
            )
            v = tl.load(
                v_ptrs, mask=(offs_n[:, None] < seq_len) & (offs_d[None, :] < head_dim), other=0.0
            )
