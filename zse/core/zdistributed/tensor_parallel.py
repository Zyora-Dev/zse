...
def all_gather(
    tensor: torch.Tensor, dim: int = -1, group: Optional[dist.ProcessGroup] = None
) -> torch.Tensor:
...
class RowParallelQuantized(nn.Module):
    ...
    def __init__(
        ...
        if original_layer.weight_packed is not None:
            self.layer.weight_packed = original_layer.weight_packed[
                :, start_packed:end_packed
            ].contiguous()
        else:
            self.layer.weight_packed = None
        if original_layer.weight_scales is not None:
            self.layer.weight_scales = original_layer.weight_scales[
        ...
        if original_layer._cached_weight is not None:
            ...
            self.layer._cached_weight = original_layer._cached_weight[
                :, start_in:end_in
            ].contiguous()
    ...
    def _fix_attention_config(self, model: nn.Module):
        ...
        for name, module in model.named_modules():
            ...
            if hasattr(module, "num_heads") and hasattr(module, "head_dim"):
                if (
                    new_num_heads is not None
                    and getattr(module, "num_heads", None) == orig_num_heads
                ):
                ...
                if (
                    hasattr(module, "num_key_value_groups")
                    and new_num_heads is not None
                    and new_kv_heads is not None
                ):
