...
def _worker_main(
    ...
    try:
        ...
        model, tokenizer = _load_model_shard(model_path, rank, world_size, tp_group)
        ...
        while True:
            ...
            try:
                if cmd["action"] == "forward":
                    ...
                    print(
                        f"[Worker {rank}] generate() start, input shape: {input_ids.shape}",
                        flush=True,
                    )
                    ...
                    print(
                        f"[Worker {rank}] generate() done, output shape: {output_ids.shape}",
                        flush=True,
                    )
