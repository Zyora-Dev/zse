...
class SparseMaskGenerator:
    ...
    def _generate_dense(self, seq_len: int) -> SparseMask:
        ...
        if self.pattern.window_size > 0:
            for i in range(seq_len):
                start = (
                    max(0, i - self.pattern.window_size + 1)
                    if self.pattern.causal
                    else max(0, i - self.pattern.window_size // 2)
                )
                end = (
                    i + 1
                    if self.pattern.causal
                    else min(seq_len, i + self.pattern.window_size // 2 + 1)
                )
        ...
        if self.pattern.num_random > 0:
            ...
            for i in range(seq_len):
                ...
                if num_random > 0:
                    random_indices = torch.randperm(max_k, generator=generator, device=self.device)[
                        :num_random
                    ]
        ...
        global_indices = torch.tensor(
            sorted(set(global_indices)), dtype=torch.long, device=self.device
        )
    ...
    def _generate_block_sparse(self, seq_len: int) -> SparseMask:
        ...
        # Last N blocks
        for i in range(
            num_blocks - (self.pattern.num_global_end + block_size - 1) // block_size, num_blocks
        ):
            global_blocks.add(i)
    ...
    def update_for_decode(self, mask: SparseMask, new_seq_len: int) -> SparseMask:
        """
...
def create_causal_sliding_window_mask(
    seq_len: int, window_size: int, device: torch.device = None
) -> torch.Tensor:
    """
