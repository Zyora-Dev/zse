...
class SparsePattern:
    ...
    @classmethod
    def sliding_window(
        cls, window_size: int = 512, causal: bool = True, max_seq_len: int = 32768
    ) -> "SparsePattern":
...
class PatternConfig:
    ...
    @classmethod
    def alternating(
        cls, pattern_a: SparsePattern, pattern_b: SparsePattern, num_layers: int
    ) -> "PatternConfig":
        """Alternate between two patterns."""
        layer_patterns = {i: pattern_a if i % 2 == 0 else pattern_b for i in range(num_layers)}
        return cls(default_pattern=pattern_a, layer_patterns=layer_patterns)
    ...
    def dense_sparse(
        ...
        layer_patterns = {
            i: dense if i < dense_layers else sparse_pattern for i in range(num_layers)
        }
        return cls(default_pattern=sparse_pattern, layer_patterns=layer_patterns)
