...
class zSparseAttention(nn.Module):
    ...
    def _expand_kv(self, kv: torch.Tensor) -> torch.Tensor:
        ...
        kv = kv.unsqueeze(2).expand(batch, num_kv_heads, gqa_factor, seq_len, head_dim)
...
def create_sparse_attention(
    pattern_name: str = "sliding_window", num_heads: int = 32, head_dim: int = 128, **pattern_kwargs
) -> zSparseAttention:
    """
