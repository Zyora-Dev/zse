...
class SpeculativeDecoder:
    ...
    def _adapt_speculation_length(self) -> None:
        ...
        recent_rate = (
            sum(self._acceptance_history[-self.config.adaptation_window :])
    ...
    def speculative_step(
        ...
        if all_accepted:
            ...
            try:
                ...
                new_draft_past = (
                    self._truncate_kv_cache(new_draft_past, accepted_len - 1)
                    if num_accepted > 0
                    else draft_past
                )
            except Exception:
        ...
        with self._lock:
            ...
            if self.stats["speculation_steps"] > 0:
                self.stats["avg_acceptance_rate"] = self.stats["accepted_tokens"] / (
