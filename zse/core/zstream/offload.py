...
class OffloadManager:
    ...
    def offload_to_cpu(
        ...
        with self._lock:
            ...
            if pinned and self.use_pinned_memory:
                cpu_state_dict = {k: v.cpu().pin_memory() for k, v in state_dict.items()}
            else:
    ...
    def get_tier_usage_gb(self) -> Dict[str, float]:
        ...
        return {tier.value: usage / (1024**3) for tier, usage in self._tier_usage.items()}
    ...
    def should_offload_from_cpu(self) -> bool:
        ...
        cpu_usage = self._tier_usage[StorageTier.CPU] + self._tier_usage[StorageTier.CPU_PINNED]
