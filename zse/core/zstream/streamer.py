...
class LayerStreamer:
    ...
    def _discover_layers(self):
        ...
        print(
            f"[zStream] Discovered {self.num_layers} layers, "
    ...
    def calculate_optimal_window(self) -> int:
        ...
        window = max(
            self.config.min_window_size, min(capacity, self.config.max_window_size, self.num_layers)
        )
    ...
    def _on_critical_pressure(self, state: GPUMemoryState):
        ...
        with self._window_lock:
            ...
            while (
                len(self._gpu_window) > self.config.min_window_size
                and self.memory_tracker.should_offload()
            ):
                self._evict_oldest()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        total_hits = self.stats["gpu_hits"] + self.stats["cpu_hits"] + self.stats["disk_hits"]
