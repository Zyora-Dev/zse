...
if TRITON_AVAILABLE:
    ...
    def _int8_matmul_dequant_kernel(
        ...
        for k in range(0, K, BLOCK_K):
            ...
            w_int8 = tl.load(
                w_ptrs, mask=((k + offs_k[:, None]) < K) & (offs_n[None, :] < N), other=0
            )
...
class QuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        if quant_type == QuantType.INT8:
            ...
            num_groups = in_features // group_size
            self.register_buffer(
                "weight_quantized", torch.zeros(out_features, packed_size, dtype=torch.uint8)
            )
            self.register_buffer(
                "weight_scale", torch.zeros(out_features, num_groups, dtype=torch.float16)
            )
        else:
