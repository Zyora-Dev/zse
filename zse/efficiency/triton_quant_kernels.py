...
def int8_fused_matmul(
    ...
    # Grid
    grid = lambda META: (
        triton.cdiv(x.shape[0], META["BLOCK_M"]) * triton.cdiv(N, META["BLOCK_N"]),
    )
    ...
    _int8_fused_matmul_kernel[grid](
        ...
        scale,
        out,
        bias if bias is not None else x,  # dummy for bias
        x.shape[0],
        N,
        K,
        x.stride(0),
...
@triton.autotune(
    configs=[
        triton.Config(
            {"BLOCK_M": 128, "BLOCK_N": 128, "BLOCK_K": 64, "GROUP_M": 8}, num_stages=3, num_warps=8
        ),
        triton.Config(
            {"BLOCK_M": 64, "BLOCK_N": 128, "BLOCK_K": 32, "GROUP_M": 8}, num_stages=4, num_warps=4
        ),
        ...
        triton.Config(
            {"BLOCK_M": 64, "BLOCK_N": 64, "BLOCK_K": 32, "GROUP_M": 8}, num_stages=4, num_warps=4
        ),
        triton.Config(
            {"BLOCK_M": 32, "BLOCK_N": 64, "BLOCK_K": 32, "GROUP_M": 8}, num_stages=5, num_warps=2
        ),
...
def _int4_fused_matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_K)):
        ...
        w_packed = tl.load(
            w_packed_ptrs, mask=(offs_n[:, None] < N) & (k_packed[None, :] < K // 2), other=0
        )
...
def int4_fused_matmul(
    ...
    _int4_fused_matmul_kernel[grid](
        ...
        w_packed.stride(1),
        scale.stride(0),
        scale.stride(1) if scale.ndim > 1 else 0,
        out.stride(0),
        out.stride(1),
        HAS_BIAS=bias is not None,
    )
...
class FusedQuantizedLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        if quant_type == "int8":
            ...
            self.register_buffer(
                "scale", torch.zeros(out_features, num_groups, dtype=torch.float16)
            )
        else:
