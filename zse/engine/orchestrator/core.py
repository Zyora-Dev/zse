...
class IntelligenceOrchestrator:
    ...
    def __init__(
        ...
        if self._is_cpu:
            ...
            if quantization in ("int4", "int8"):
                print(
                    f"⚠️  Note: {quantization.upper()} quantization requires GPU. Using FP32 for CPU."
                )
    ...
    @classmethod
    def auto(
        cls, model_name: str, device: str = "auto", draft_model: Optional[str] = None
    ...
    @classmethod
    def multi_gpu(
        cls, model_name: str, quantization: str = "auto", gpu_ids: Optional[list] = None
    ) -> "IntelligenceOrchestrator":
        """
    ...
    def load(self, verbose: bool = True) -> "IntelligenceOrchestrator":
        ...
        if verbose:
            ...
            if self._tp_size > 1:
                ...
                gpu_str = (
                    f"GPUs {self._gpu_ids}" if self._gpu_ids else f"All {self._gpu_count} GPUs"
                )
        ...
        if model_path.exists():
            ...
            if model_path.is_dir():
                ...
                if tokenizer_config.exists():
                    ...
                    if (model_path / "model.safetensors").exists() or (
                        model_path / "model.zse"
                    ).exists():
                    ...
                    if zse_file.exists():
                        ...
                        raise ValueError(
                            f"No tokenizer found in {model_path}. Directory must contain tokenizer_config.json or model.zse"
                        )
        ...
        if self._tp_size > 1:
                ...
                if safetensors_file.exists():
                    ...
                    if config_file.exists():
                        ...
                        model_config = AutoConfig.from_pretrained(
                            str(zse_path), trust_remote_code=True
                        )
            ...
            dtype = (
                torch.float32 if (self.quantization == "fp32" or self._is_cpu) else torch.float16
            )
            self.model = AutoModelForCausalLM.from_pretrained(
                self.model_name,
                torch_dtype=dtype,
    ...
    def _load_draft_model(self, device: str, verbose: bool = True) -> nn.Module:
        ...
        if draft_path.exists() and (
            draft_path.suffix == ".zse"
            or (draft_path.is_dir() and (draft_path / "model.zse").exists())
        ):
    ...
    def benchmark(
        self, prompt: str = "Write a hello world in Python", tokens: int = 100
    ) -> InferenceStats:
        """
