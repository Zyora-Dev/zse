...
class ContinuousBatchingScheduler:
    ...
    def _can_allocate(self, request: InferenceRequest) -> bool:
        ...
        current_tokens = sum(sg.request.total_len for sg in self.running.values())
    ...
    def get_batch_inputs(
        ...
        for seq_group in seq_groups:
            if seq_group.is_prefill:
                ...
                # Decode: single token
                tokens = (
                    [seq_group.request.generated_tokens[-1]]
                    if seq_group.request.generated_tokens
                    else seq_group.request.prompt_tokens[-1:]
                )
