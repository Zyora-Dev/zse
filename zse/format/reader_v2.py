...
class QuantizedLinearZSE(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "weight_packed", torch.zeros(out_features, in_features // 2, dtype=torch.uint8)
        )

        # Scales: [out_features, num_groups]
        num_groups = (in_features + group_size - 1) // group_size
        self.register_buffer(
            "weight_scales", torch.zeros(out_features, num_groups, dtype=torch.float16)
        )
    ...
    def _convert_triton_to_bnb_fallback(self):
        ...
        for i in range(0, self.out_features, 256):
            ...
            num_rows = end - i
            eye = torch.zeros(
                (num_rows, K), dtype=torch.float16, device=self._triton_v2_weight.device
            )
            identity = torch.eye(
                num_rows, K, dtype=torch.float16, device=self._triton_v2_weight.device
            )
        ...
        weight_fp16 = dequantize_int4_zse(
            w_packed_nk, scales_ng, self.group_size, dtype=torch.float16
        )
        if weight_fp16.shape[1] > self.in_features:
            weight_fp16 = weight_fp16[:, : self.in_features]
...
def _convert_model_to_backend(model: nn.Module, backend: str) -> None:
    ...
    if backend in ("triton", "auto"):
        if is_triton_v2_available():
            ...
            raise RuntimeError(
                f"Triton backend requested but not available: {get_triton_v2_error()}"
            )
