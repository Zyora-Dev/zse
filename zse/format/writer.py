...
class ZSEWriter:
    ...
    def _build_header_from_config(
        ...
        self.header = ZSEHeader(
            ...
            num_key_value_heads=getattr(
                config, "num_key_value_heads", getattr(config, "num_attention_heads", 0)
            ),
    ...
    def _serialize_tokenizer(self, tokenizer: Any) -> bytes:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            packed = {k: base64.b64encode(v).decode("ascii") for k, v in tokenizer_data.items()}
    ...
    def _write_file(
        ...
        with open(self.output_path, "wb") as f:
            ...
            for layer_idx in sorted(layer_tensors.keys()):
                ...
                self.header.layer_groups.append(
                    LayerGroup(
                        layer_idx=layer_idx,
    ...
    def _write_tensor(
        ...
        should_quantize = (
            self.config.quantization in ("int4", "int8")
            and len(tensor.shape) == 2
            and tensor.shape[0] > 64
            and tensor.shape[1] > 64  # Skip small tensors
            and "weight" in name
            and not any(
                skip in name for skip in ["embed", "norm", "lm_head"]
