...
if _TRITON_AVAILABLE:
    ...
    def _int4_gemm_v3(
        ...
        pid_n = (pid % num_pid_in_group) // group_size_m + (
            pid // num_pid_in_group // ((num_pid_m + num_pid_in_group - 1) // num_pid_in_group)
        ...
        for k in range(0, tl.cdiv(K, BLOCK_K)):
            ...
            b_packed = tl.load(
                B_ptr
                + ((k_start // 2) + tl.arange(0, BLOCK_K // 2))[:, None] * stride_bk
            ...
            scales = tl.load(
                scales_ptr + group_idx * stride_sg + offs_n * stride_sn, mask=offs_n < N, other=1.0
            )
            ...
            a_even = tl.load(
                ...
                + (k_start + tl.arange(0, BLOCK_K // 2) * 2)[None, :] * stride_ak,
            ...
            a_odd = tl.load(
                A_ptr
                + offs_m[:, None] * stride_am
                + (k_start + tl.arange(0, BLOCK_K // 2) * 2 + 1)[None, :] * stride_ak,
    ...
    def _int4_gemv_v3(
        ...
        for k in range(0, tl.cdiv(K, BLOCK_K)):
            ...
            w_packed = tl.load(
                W_ptr
                + ((k_start // 2) + tl.arange(0, BLOCK_K // 2))[:, None] * stride_wk
            ...
            group_idx = k_start // group_size
            scales = tl.load(
                scales_ptr + group_idx * stride_sg + offs_n * stride_sn, mask=n_mask, other=1.0
            ).to(tl.float32)
            ...
            x_even = tl.load(
                x_ptr + k_start + tl.arange(0, BLOCK_K // 2) * 2,
                mask=(k_start + tl.arange(0, BLOCK_K // 2) * 2) < K,
                other=0.0,
            ).to(tl.float32)
            x_odd = tl.load(
                x_ptr + k_start + tl.arange(0, BLOCK_K // 2) * 2 + 1,
                mask=(k_start + tl.arange(0, BLOCK_K // 2) * 2 + 1) < K,
                other=0.0,
            ).to(tl.float32)
    ...
    def _int4_gemv_persistent(
        ...
        for block_id in range(pid, num_n_blocks, num_sms):
            ...
            for g in range(0, tl.cdiv(K, group_size)):
                ...
                scales = tl.load(
                    scales_ptr + g * stride_sg + offs_n * stride_sn, mask=n_mask, other=1.0
                ).to(tl.float32)
