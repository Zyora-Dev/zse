...
class ModelDiscovery:
    ...
    def _get_client(self):
        ...
        if not HAS_HTTPX:
            raise ImportError(
                "httpx is required for model discovery. Install with: pip install httpx"
            )
    ...
    def _parse_model_info(self, data: Dict[str, Any]) -> HFModelInfo:
        ...
        if config:
            if isinstance(config, dict):
                architecture = (
                    config.get("model_type") or config.get("architectures", [None])[0]
    ...
    def check_compatibility(self, model_id: str) -> Dict[str, Any]:
        ...
        if info.architecture in SUPPORTED_ARCHITECTURES:
            ...
            result["issues"].append(
                f"Architecture '{info.architecture}' not explicitly listed but may work"
            )
        else:
        ...
        has_safetensors = any(
            f.get("path", "").endswith(".safetensors") for f in files if f.get("type") == "file"
        )
