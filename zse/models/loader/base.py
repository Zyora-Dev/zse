...
class ModelInfo:
    ...
    def from_config(cls, config: Dict[str, Any], name: str = "unknown") -> "ModelInfo":
        ...
        # GQA support
        num_kv_heads = config.get("num_key_value_heads", config.get("num_kv_heads", num_heads))
...
class BaseModelLoader(ABC):
    ...
    def _quantize_tensor(
        ...
        if self.config.quantize_layers:
            should_quantize = any(pattern in name for pattern in self.config.quantize_layers)
