...
class MMapTensorLoader:
    ...
    def load_tensor(
        ...
        tensor = (
            torch.frombuffer(memoryview(self._mmap)[tensor_start:tensor_end], dtype=torch_dtype)
            .reshape(shape)
            .clone()
        )  # clone() to own the data after mmap closes
...
class SafetensorsLoader(BaseModelLoader):
    ...
    def __init__(self, config: Optional[LoadConfig] = None):
        ...
        if not SAFETENSORS_AVAILABLE:
            logger.warning(
                "safetensors package not installed. Install with: pip install safetensors"
            )
